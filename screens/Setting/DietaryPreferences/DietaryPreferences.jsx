...
const DietaryPreferences = ({ navigation }) => {
  ...
  const handleFetchPersonalize = async () => {
    ...
    try{
        const response = await fetch(`${HOST}/personalize/user/${userInfo?.id}`, {headers})
    ...
    try{
        ...
        if (data.length > 0){
            ...
            const refactorData = data.map((m) => {
                const { id, name } = m;
                const isExist = personalize?.diets?.some(item => item.id === id) ? true : false;
        ...
        if (data.length > 0){
          const listAllergies = []
            const refactorData = data.map((m) => {
                ...
                const isExist = personalize?.allergies?.some(item => item.id === id) ? true : false;
                if (isExist) {
  ...
  const handleFetchCuisines = async (personalize) => {
    ...
    try{
        ...
        if (data.length > 0){
          ...
          const refactorData = data.map((m) => {
              const { id, name } = m;
              const isExist = personalize?.cuisines?.some(item => item.id === id) ? true : false;
              if (isExist) {
  ...
  const handleIsChange = () => {
    ...
    const dietsMatch = diets.length === dietsIds.length && diets.every(dietId => dietsIds.includes(dietId));

    // Check if all elements in allergies match allergiesIds
    const allergiesMatch = allergies.length === allergiesIds.length && allergies.every(allergyId => allergiesIds.includes(allergyId));
    // Check if all elements in cuisines match cuisinesIds
    const cuisinesMatch = cuisines.length === cuisinesIds.length && cuisines.every(cuisineId => cuisinesIds.includes(cuisineId));
  ...
  const handleChangePersonalize = (idPer, id) => {
    if (idPer === 1) {
      ...
      setAllergies(prev => 
        prev.includes(id) ? prev.filter(allergyId => allergyId !== id) : [...prev, id]
      );
    } else if (idPer === 3) {
      setCuisines(prev => 
        prev.includes(id) ? prev.filter(cuisineId => cuisineId !== id) : [...prev, id]
      );
    }
  };
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          ...
          {mockData.map((item) => {
            return <DietaryItem key={item.id} data={item} handleChangePersonalize={handleChangePersonalize}/>
          })}
        </View>
      </ScrollView>
      <View style={styles.updateBtnContainer}>
        <TouchableOpacity
          ...
          onPress={handlePress}
          style={[styles.updateBtn, {backgroundColor: handleIsChange() || loading ? theme.colors.darkGray : theme.colors.secondary}]}>
          {
            loading ? <ActivityIndicator size={"small"} color={"white"} /> :
            <Text style={{color: 'white', fontSize: 15, fontWeight: '600'}}>Update Preferences</Text>
          }
        </TouchableOpacity>
      </View>
    </SafeAreaView>
