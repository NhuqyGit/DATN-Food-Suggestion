  ...
  useEffect(() => {
    const createPersonalize = async () => {
      ...
      try {
        ...
        const cuisineIds = newQues[0].listAns
          .filter((cuisine) => cuisine.isSelect)
          .map((cuisine) => cuisine.id)
        const dietIds = newQues[1].listAns
          .filter((diet) => diet.isSelect)
          .map((diet) => diet.id)
        const allergyIds = newQues[2].listAns
        ...
        const response = await axios.post(`${HOST}/personalize`, bodyData, {
          headers,
        })
