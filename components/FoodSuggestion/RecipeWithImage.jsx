...
const RecipeWithImage = ({response}) => {
    ...
    const listVideo = responseChat?.video?.length > 0 ? (
        responseChat.video.map((v, index) => (
            <View
                key={index}
                style={{gap: 3, marginBottom: 5, alignItems: 'flex-start'}}>
                <Text style={{marginRight: 5, color: 'white'}}>Link {index + 1}</Text>
                <TouchableOpacity
                    onPress={() => handlePress(v.url)}>
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={{fontWeight: "500", color: "white", fontSize: 15, alignSelf: 'flex-start', marginTop: 10}}>Các link video hướng dẫn tham khảo:</Text>
            {/* {
