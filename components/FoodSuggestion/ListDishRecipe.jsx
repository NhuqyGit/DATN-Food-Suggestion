...
const ListDishRecipe = ({response}) => {
    ...
    const findIndexRecipe = () => {
        const index = responseChat?.dishList?.findIndex(
            (r) => r.id === id
        );
        return index
    }
        ...
        <View>
            <ScrollView
                ...
                {responseChat?.dishList?.map((item) => (
                    ...
                    <DishRecipe key={item.id} item={item} handleSetId={handleSetId} id={id}/> 
                ))}
            </ScrollView>

            <View style={{backgroundColor: theme.colors.lightGray, padding: id ? 8 : 0}}>
